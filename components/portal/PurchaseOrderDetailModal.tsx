...
const PurchaseOrderDetailModal: React.FC<{ po: PurchaseOrder; isOpen: boolean; onClose: () => void }> = ({ po, isOpen, onClose }) => {
    const { t, lang } = useLocalization();
    const { companies, loggedInSupplier, parts, currencies } = useAppStore();
    const companyProfile = companies.find(c => c.id === loggedInSupplier?.companyId);

    const partsMap = new Map(parts.map(p => [p.id, p]));
    const currency = currencies.find(c => c.id === po?.currencyId);

    if (!isOpen || !po || !companyProfile || !loggedInSupplier) return null;

    const handlePrint = () => window.print();

    const formatCurrency = (amount: number) => {
        return `${amount.toLocaleString('en-US', { minimumFractionDigits: 2 })} ${currency?.symbol || ''}`;
    };
                ...
                <div className="flex-grow overflow-auto p-1 bg-muted/20 print-parent">
                    <div className="print-area bg-card text-card-foreground p-6">
                        <section className="flex justify-between items-start pb-6 border-b-2 border-primary">
                             <div>
                                <h3 className="font-semibold text-muted-foreground">{t('to')}:</h3>
                        ...
                        <section className="grid grid-cols-2 gap-6 my-6">
                            <div>
                                <h3 className="font-semibold text-muted-foreground">{t('from')}:</h3>
